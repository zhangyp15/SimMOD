...
class FCOSMono3DHead(AnchorFreeMono3DHead):
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.target_code_size = self.bbox_code_size + int(self.pred_corner) * (
            self.num_corner * 3
        )
    ...
    def _init_layers(self):
        ...
        self.num_attribute_scales = (
            3 + int(self.pred_bbox2d_training) + int(self.pred_corner)
        )
        self.scales = nn.ModuleList(
            [
                nn.ModuleList([Scale(1.0) for _ in range(self.num_attribute_scales)])
    ...
    def forward_single(self, x, scale, stride):
        ...
        # bbox_pred, (batch, num_pred, h / s, w / s), num_pred includes 2d_offset(2), depth(1), 3d_size(3), rot(1), velo(2)
        ...
        if self.pred_bbox2d_training:
            scale_bbox2d = scale[3]
            bbox_pred[:, 9:13] = scale_bbox2d(clone_bbox_pred[:, 9:13]).float().exp()

        if self.pred_corner:
            scale_corner = scale[4]
            bbox_pred[:, 13:29] = scale_corner(clone_bbox_pred[:, 13:29]).float()
    ...
    def loss(
        ...
        (
            ...
            gt_corners_2d_list=gt_corners_2d,
        )
        ...
        if num_pos > 0:
            ...
            loss_bbox2d = None
            if self.pred_bbox2d_training:
                loss_bbox2d, pred_iou_bbox2d = self.loss_bbox2d(
                    pos_bbox_preds[:, 9:13], pos_bbox_targets_2d
                )
                loss_bbox2d = loss_bbox2d.sum() / equal_weights.sum()

                # print('The mean 2D IoU of predicted bounding boxes = {:.4f}'.format(pred_iou_bbox2d.mean().item()))

            loss_corner = None
            if self.pred_corner:
                corner_targets_2d_valid_mask = pos_bbox_targets_3d[:, 25:33]
                corner_weights = (
                    bbox_weights[:, 13:29]
                    * corner_targets_2d_valid_mask.repeat(1, 2).float()
                )

                loss_corner = self.loss_bbox(
                    pos_bbox_preds[:, 13:29],
                    pos_bbox_targets_3d[:, 9:25],
                    weight=corner_weights,
                    avg_factor=corner_targets_2d_valid_mask.sum(),
                )
            ...
            if self.pred_bbox2d_training:
                loss_bbox2d = pos_bbox_preds[:, 9:13].sum()

            if self.pred_corner:
                loss_corner = pos_bbox_preds[:, 13:29].sum()
        ...
        if loss_corner is not None:
            loss_dict["loss_corner"] = loss_corner
    ...
    def _get_bboxes_single(
        ...
        cam_intrinsic[: view.shape[0], : view.shape[1]] = mlvl_centers2d.new_tensor(
            view
        )
        mlvl_bboxes[:, 6] = (
            torch.atan2(mlvl_centers2d[:, 0] - cam_intrinsic[0, 2], cam_intrinsic[0, 0])
            + mlvl_bboxes[:, 6]
        )
    ...
    def get_targets(
        ...
        if gt_corners_2d_list is None:
            gt_corners_2d_list = [None] * len(gt_labels_list)
        ...
        for i in range(num_levels):
            ...
            bbox_targets_2d = torch.cat(
                [bbox_targets_2d[i] for bbox_targets_2d in bbox_targets_list]
            )

            if self.norm_on_bbox:
                # normalize center2d_offset
                bbox_targets_3d[:, :2] = bbox_targets_3d[:, :2] / self.strides[i]
                # normalize keypoint_offsets
                bbox_targets_3d[:, 9:25] = bbox_targets_3d[:, 9:25] / self.strides[i]
                bbox_targets_2d = bbox_targets_2d / self.strides[i]

            concat_lvl_bbox_targets_3d.append(bbox_targets_3d)
            concat_lvl_bbox_targets_2d.append(bbox_targets_2d)
    ...
    def _get_target_single(
        ...
        if num_gts == 0:
            return (
                ...
                gt_bboxes_3d.new_zeros((num_points, self.target_code_size)),
        ...
        if self.pred_corner:
            gt_corners_2d = gt_corners_2d[None].expand(
                num_points, num_gts, self.num_corner, 3
            )
