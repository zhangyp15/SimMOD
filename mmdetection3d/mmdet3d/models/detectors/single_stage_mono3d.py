...
class SingleStageMono3DDetector(SingleStageDetector):
    ...
    def forward_train(
        ...
        if debug:
            ...
            img_mean, img_std = img_norm_cfg["mean"], img_norm_cfg["std"]
            img_mean = torch.from_numpy(img_mean).type_as(img).view(1, -1, 1, 1)
            img_std = torch.from_numpy(img_std).type_as(img).view(1, -1, 1, 1)
            recover_img = img * img_std + img_mean
            recover_img = (
                recover_img.permute(0, 2, 3, 1).detach().cpu().numpy().astype(np.uint8)
            )
            recover_img = recover_img[..., [2, 1, 0]]
            save_path = "debug_figs/"
            ...
            for i in range(recover_img.shape[0]):
                img_i = np.ascontiguousarray(recover_img[i].astype(np.uint8))
                gt_bboxes_i = gt_bboxes[i]
                gt_bboxes_3d_i = gt_bboxes_3d[i]
                gt_corners_3d_i = gt_bboxes_3d_i.corners
                Ki = (
                    torch.tensor(img_metas[i]["cam2img"])
                    .type_as(gt_corners_3d_i)
                    .t()[None]
                )
                gt_corners_2d_i = torch.matmul(gt_corners_3d_i, Ki)
                gt_corners_2d_i = gt_corners_2d_i[..., :2] / gt_corners_2d_i[..., 2:]
                gt_corners_2d_i = gt_corners_2d_i.detach().cpu().numpy()
                img_with_corner_i = plot_rect3d_on_img(
                    img_i,
                    num_rects=gt_corners_2d_i.shape[0],
                    rect_corners=gt_corners_2d_i,
                )
                img_with_bbox_i = plot_bbox2d_on_img(
                    img_i, bboxes=gt_bboxes_i.detach().cpu().numpy()
                )
                ...
                plt.axis("off")
                plt.subplot(222)
                plt.imshow(img_with_corner_i)
                plt.axis("off")
                plt.subplot(223)
                plt.imshow(img_with_bbox_i)
                plt.axis("off")
                plt.savefig(
                    "{}/{}".format(
                        save_path, os.path.basename(img_metas[i]["filename"])
                    )
                )
                plt.close(0)
        ...
        for i in range(len(outs_list[0])):  # for each prediction
            ...
            for j in range(len(outs_list[0][i])):
                ...
                avg_feats = torch.mean(
                    torch.cat([x[i][j] for x in outs_list]), dim=0, keepdim=True
                )
    ...
    def show_results(self, data, result, out_dir):
        ...
        for batch_id in range(len(result)):
            ...
            show_multi_modality_result(
                img, None, pred_bboxes, cam2img, out_dir, file_name, "camera", show=True
            )
