...
def show_result(
    points, gt_bboxes, pred_bboxes, out_dir, filename, show=False, snapshot=False
):
    ...
    if show:
        ...
        show_path = (
            osp.join(result_path, f"{filename}_online.png") if snapshot else None
        )
        vis.show(show_path)
...
def show_multi_modality_result(
    ...
    if show:
        ...
        if pred_bboxes is not None:
            show_img = draw_bbox(
                pred_bboxes, show_img, proj_mat, img_metas, color=pred_bbox_color
            )
