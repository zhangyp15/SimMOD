...
def plot_bbox2d_on_img(img, bboxes, color=(0, 255, 0), thickness=1):
    """Plot the 2D bounding boxes on 2D images.

    Args:
        img (numpy.array): The numpy array of image.
        bboxes (numpy.array): Coordinates of 2D bounding boxes in [x1, y1, x2, y2]. Should be in the shape of [num_boxes, 4].
    ...
    for i in range(bboxes.shape[0]):
        bbox_i = bboxes[i].astype(np.int)
        cv2.line(
            img,
            (bbox_i[0, 0], bbox_i[0, 1]),
            (bbox_i[1, 0], bbox_i[0, 1]),
            color,
            thickness,
            cv2.LINE_AA,
        )
