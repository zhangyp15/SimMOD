...
class DefaultFormatBundle(object):
    ...
    def __call__(self, results):
        ...
        if "cam_anno_infos" in results:
            ...
            for index, cam_anno_info in enumerate(results["cam_anno_infos"]):
                for key in mono_ann_keys:
                    if key == "gt_bboxes_3d":
                        results["cam_anno_infos"][index][key] = DC(
                            cam_anno_info[key], cpu_only=True
                        )
                    else:
                        results["cam_anno_infos"][index][key] = DC(
                            to_tensor(cam_anno_info[key])
                        )

                if "gt_tokens" in results["cam_anno_infos"][index]:
                    results["cam_anno_infos"][index].pop("gt_tokens")
