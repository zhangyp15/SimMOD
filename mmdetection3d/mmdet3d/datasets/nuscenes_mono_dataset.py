...
class NuScenesMonoDataset(CocoDataset):
    ...
    def _parse_ann_info(self, img_info, ann_info):
        ...
        """
        确认 fcos3d 使用了 2.5D 中心，而且选择在 2.5D 中心进行 center-sampling 得到回归目标，在这种情况下，截断物体根本无法得到训练，因为 2.5D 中心位于图像外，此时 center-sampling 无法得到得到 feature-map内的有效坐标 
        """

        # print(centers2d.min(axis=0), centers2d.max(axis=0))
        ...
        if len(gt_bboxes_cam3d.tensor) > 0:
            ...
            # print('valid number of key-points = {} / {} = {:.4f}'.format(valid_mask.sum(), valid_mask.size, float(valid_mask.sum()) / valid_mask.size))
...
def nusc_box_to_cam_box3d(boxes):
    ...
    rots = torch.Tensor(np.array([b.orientation.yaw_pitch_roll[0] for b in boxes])).view(-1, 1)
