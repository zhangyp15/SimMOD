...
class SwinTransformer(BaseModule):
    ...
    def __init__(
        ...
        # Add a norm layer for each output, requires_grad is set to false for output
        output_norm_cfg = norm_cfg.copy()
        output_norm_cfg["requires_grad"] = False
        for i in out_indices:
            layer = build_norm_layer(output_norm_cfg, self.num_features[i])[1]
            layer_name = f"norm{i}"
