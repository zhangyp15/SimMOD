...
class Detr3D(MVXTwoStageDetector):
    ...
    def forward_pts_train(
        ...
        # loss_sum = sum(
        #     _loss.item() for _key, _loss in losses.items() if _key.find("loss") >= 0
        # )

        # print("detr3d loss sum = {:.4f}".format(loss_sum))
    ...
    def forward_train(
        ...
        # multi-stage features in [torch.Size([1, 6, 256, 116, 200]), torch.Size([1, 6, 256, 58, 100]), torch.Size([1, 6, 256, 29, 50]), torch.Size([1, 6, 256, 15, 25])]
    ...
    def simple_test_pts(self, x, img_metas, rescale=False, raw_img=None):
        """Test function of point cloud branch."""
        outs = self.pts_bbox_head(x, img_metas, raw_img=raw_img)
    ...
    def simple_test(self, img_metas, img=None, rescale=False, raw_img=None):
        ...
        bbox_pts = self.simple_test_pts(img_feats, img_metas, rescale=rescale, raw_img=raw_img)
